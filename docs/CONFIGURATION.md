...
| `AUTO_PROTECT_ON_DB_UPDATE` | Set to `'true'` to enable `autoProtectOnUpdate` for every model that doesn't set the `extensions.autoProtectOnUpdate` option explicitly — see [Data Protection: automatic update-time protection](./DATA-PROTECTION.md#automatic-update-time-protection-autoprotectonupdate).         | disabled                 |
