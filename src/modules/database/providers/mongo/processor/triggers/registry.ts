...
const staticStore = new Map<string, Triggers>()
const persistedStore = new Map<string, Triggers>()
...
const defaultSuppressed = new Set<string>()
 ...
 */
export const setStaticTriggers = (modelName: string, triggers?: Triggers): void => {
  if (!triggers) {
    staticStore.delete(modelName)
    return
  }
  staticStore.set(modelName, triggers)
}

/**
 * Returns every model name with a static `extensions.triggers` layer registered, and that layer's
 * triggers — used at connector startup to decide which models still need a default persisted entry
 * seeded for them (see `loadPersistedTriggersOnStart`).
 */
export const getStaticTriggerEntries = (): Array<[string, Triggers]> => [...staticStore.entries()]

/**
 * Replaces a model's persisted-triggers layer (loaded from the persisted triggers collection —
 * see `registerTriggersModel`) with `triggers`.
 *
 ...
 */
export const setPersistedTriggers = (modelName: string, triggers: Triggers): void => {
  persistedStore.set(modelName, triggers)
}

/**
 * Marks a model's static `extensions.triggers` layer as permanently replaced by a persisted
 * **default** entry (`isDefault: true`, auto-seeded from that same static configuration).
 *
 ...
 */
export const setDefaultSuppressed = (modelName: string): void => {
  defaultSuppressed.add(modelName)
}

/**
 * Clears the entire persisted-triggers layer, for every model, and the default-suppression set —
 * call this before reloading from the persisted triggers collection, so the reload fully reflects
 * the database's current state (including entries that became inactive or were removed) instead of
 * merging on top of a previous, possibly-stale load. Never touches the static layer itself.
 */
export const resetPersistedTriggers = (): void => {
  persistedStore.clear()
  defaultSuppressed.clear()
}

/**
 * Returns a model's current effective triggers, or `undefined` if nothing applies to it:
 *
 ...
 */
export const getTriggers = (modelName: string): Triggers | undefined => {
  const persistedTriggers = persistedStore.get(modelName)
  if (defaultSuppressed.has(modelName)) return persistedTriggers

  const staticTriggers = staticStore.get(modelName)
