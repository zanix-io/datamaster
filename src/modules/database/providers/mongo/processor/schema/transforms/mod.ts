import type { BaseCustomSchema, Transform } from 'mongo/typings/schema.ts'

import { transformClearIdMetadata } from './metadata.ts'
import { transformByDataAccess } from './data-policies.ts'
 ...
 */
export const baseTransformations = (schema: BaseCustomSchema): void => {
  ...
  const jsonTransforms: Transform[] = [transformClearIdMetadata]
  if (schema.statics._hasDataAccess()) {
    jsonTransforms.push(transformByDataAccess({ deleteMetadata: true }))
  }

  applyTransformations(schema, jsonTransforms, 'toJSON')
}
