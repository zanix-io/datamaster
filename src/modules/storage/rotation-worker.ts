 ...
 * `S3ObjectStorage` from plain, structured-cloneable `connectorOptions` (a live instance,
...
import { S3ObjectStorage } from './connector.ts'
...
export function checkEncryptionRotationStatusInWorker(
  ...
  return runCheck(new S3ObjectStorage(connectorOptions), options)
}
...
export function rotateEncryptionKeysInWorker(
  ...
  return runRotate(new S3ObjectStorage(connectorOptions), options)
}
