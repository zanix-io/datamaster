...
Deno.test({
  ...
  fn: async () => {
    ...
    const setup = new ZanixMongoConnector({
      seedModel: false,
    ...
    await DropCollection(
      db.getModel<TriggersModelAttrs>(triggersModelName),
      db,
    )
...
Deno.test({
  ...
  fn: async () => {
    ...
    await TriggersModel.create({
      ...
      active: true,
      triggers: {
        post: { created: [{ custom: { name: 'db-loaded-job-2' } }] },
      },
    })
    ...
    assertEquals(
      ...
      new Set(['static-job', 'db-loaded-job-2']),
    )
...
Deno.test({
  ...
  fn: async () => {
    ...
    const boot1 = new ZanixMongoConnector({
      seedModel: false,
      triggersModel: triggersModelName,
    })
    await boot1.isReady

    const TriggersModel1 = boot1.getModel<TriggersModelAttrs>(
      triggersModelName,
    )
    ...
    const boot2 = new ZanixMongoConnector({
      seedModel: false,
      triggersModel: triggersModelName,
    })
    await boot2.isReady

    const Model2 = boot2.getModel<any>(
      targetModelName,
      new Schema({ str: String }),
    )
    ...
    const TriggersModel2 = boot2.getModel<TriggersModelAttrs>(
      triggersModelName,
    )
    await TriggersModel2.updateOne({ model: targetModelName }, {
      $set: { active: false },
    })
    await boot2['close']()
    ...
    const boot3 = new ZanixMongoConnector({
      seedModel: false,
      triggersModel: triggersModelName,
    })
    await boot3.isReady

    const Model3 = boot3.getModel<any>(
      targetModelName,
      new Schema({ str: String }),
    )
    ...
    await DropCollection(
      boot3.getModel<TriggersModelAttrs>(triggersModelName),
      boot3,
    )
...
Deno.test({
  ...
  fn: async () => {
    ...
    const staticTriggers = {
      post: { created: [{ custom: { name: 'seed-job' } }] },
    }
    ...
    const seeded = await TriggersModel.findOne({ model: targetModelName })
      .lean()
...
Deno.test({
  ...
  fn: async () => {
    ...
    const staticTriggers = {
      post: { created: [{ custom: { name: 'seed-off-job' } }] },
    }
    ...
    await DropCollection(
      boot2.getModel<TriggersModelAttrs>(triggersModelName),
      boot2,
    )
...
Deno.test({
  ...
  fn: async () => {
    ...
    const staticTriggers = {
      post: { created: [{ custom: { name: 'original-job' } }] },
    }
    const definition = { str: String }
    ...
    await TriggersModel1.updateOne({ model: targetModelName }, {
      $set: {
        triggers: { post: { created: [{ custom: { name: 'edited-job' } }] } },
      },
    })
    await boot1['close']()
...
Deno.test({
  ...
  fn: async () => {
    ...
    const staticTriggers = {
      post: { created: [{ custom: { name: 'reseed-job' } }] },
    }
    const definition = { str: String }
    ...
    const reseeded = await TriggersModel2.findOne({ model: targetModelName })
      .lean()
...
Deno.test({
  ...
  fn: async () => {
    ...
    const a = new ZanixMongoConnector({
      seedModel: false,
      triggersModel: triggersModelName,
    })
    await a.isReady
    ...
    const a2 = new ZanixMongoConnector({
      seedModel: false,
      triggersModel: triggersModelName,
    })
    await a2.isReady
    ...
    const b = new ZanixMongoConnector({
      seedModel: false,
      triggersModel: false,
    })
    await b.isReady

    const Model = b.getModel<any>(
      targetModelName,
      new Schema({ str: String }),
      {
        extensions: {
          triggers: { post: { created: [{ custom: { name: 'static-job' } }] } },
        },
      },
    )
    await new Model({ str: 'a' }).save()
    ...
    await DropCollection(
      a2.getModel<TriggersModelAttrs>(triggersModelName),
      a2,
    )
...
Deno.test({
  ...
  fn: async () => {
    ...
    registerModel({
      ...
      definition,
      extensions: {
        triggers: { post: { created: [{ custom: { name: 'orphan-job' } }] } },
      },
    })
    const boot1 = new ZanixMongoConnector({
      seedModel: false,
    ...
    const entry = await TriggersModel.findOne({ model: targetModelName })
      .lean()
...
Deno.test({
  ...
  fn: async () => {
    ...
    registerModel({
      ...
      definition,
      extensions: {
        triggers: { post: { created: [{ custom: { name: 'old-job' } }] } },
      },
    })
    const boot1 = new ZanixMongoConnector({
      seedModel: false,
    ...
    registerModel({
      ...
      definition,
      extensions: {
        triggers: { post: { created: [{ custom: { name: 'new-job' } }] } },
      },
    })
    const boot2 = new ZanixMongoConnector({
      seedModel: false,
...
Deno.test({
  ...
  fn: async () => {
    ...
    await TriggersModel1.updateOne({ model: targetModelName }, {
      $set: {
        triggers: {
          post: { created: [{ custom: { name: 'hand-edited-job' } }] },
        },
      },
    })
    await boot1['close']()
...
Deno.test({
  ...
  fn: async () => {
    ...
    const originalFind: (...args: any[]) => any = TriggersModel.find.bind(
      TriggersModel,
    )
    TriggersModel.find = (() => ({
...
Deno.test({
  ...
  fn: async () => {
    ...
    try {
      ...
      // overload instead.
      const other = new ZanixMongoConnector({
        seedModel: false,
        triggersModel: false,
      })
      await other.isReady
      ...
      const OtherTriggersModel = other.getModel(
        triggersModelName,
        rawTriggersSchema,
      )
  ...
  fn: async () => {
    ...
    try {
      ...
      await TriggersModel.create({
        ...
        active: true,
        triggers: {
          post: { created: [{ custom: { name: 'still-works-job' } }] },
        },
      })
...
Deno.test({
  ...
  fn: async () => {
    ...
    try {
      ...
      // test already covers.
      calls.length = 0
      await TriggersModel.updateOne({ model: targetModelName }, {
        $set: { active: false },
      })
...
Deno.test({
  ...
  fn: async () => {
    ...
    const cleanup = new ZanixMongoConnector({
      seedModel: false,
      triggersModel: false,
    })
    ...
    await DropCollection(
      cleanup.getModel(triggersModelName, rawTriggersSchema),
      cleanup,
    )
    await cleanup['close']()
  },
